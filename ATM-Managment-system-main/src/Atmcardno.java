...
public class Atmcardno extends JFrame implements ActionListener {
	...
	public Atmcardno() {
		...
		l4.setBounds(250, 50, 300, 30);
		jf.add(l4);

		l2 = new JLabel("Enter ATM card no:");
		l2.setFont(f1);
		l2.setForeground(Color.WHITE);
		l2.setBounds(120, 190, 250, 30);
		jf.add(l2);

		t1 = new JTextField(20);
		t1.setBounds(370, 190, 200, 30);
		jf.add(t1);

		l3 = new JLabel("Enter PIN no:");
		l3.setFont(f1);
		l3.setForeground(Color.WHITE);
		l3.setBounds(120, 280, 250, 30);
		jf.add(l3);

		pwd = new JPasswordField(10);
		pwd.setFont(f1);
		pwd.setBounds(370, 280, 200, 30);
		jf.add(pwd);
		...
		t = new Timer(2000000000, this);// 20 minisecond
		// t.start();
	...
	public void actionPerformed(ActionEvent ae) {
		if (ae.getSource() == t) {
			...
			try {
				if (((t1.getText()).equals("")) && ((pwd.getText()).equals(""))) {
					JOptionPane.showMessageDialog(this, "Please enter ATM card no and PIN no!", "Warning",
					...
					ps = con.prepareStatement("select * from accountdetail where atmno='" + t1.getText()
							+ "' and pinno='" + pwd.getText() + "'");
					rs = ps.executeQuery();
					while (rs.next()) {
						...
						try {
							...
							if (date1.compareTo(date2) >= 0) {
								System.out.println("Expiry date of atm:" + sdf.format(date1) + "is after today date:"
										+ sdf.format(date2));
								...
								JOptionPane.showMessageDialog(this,
										"Your atm card is out of expiry date.Please take new ATM card from your home bank.",
					...
					if (foundrec == 0) {
						JOptionPane.showMessageDialog(null, "Invalid ATM card no or PIN no.", "Warning",
								JOptionPane.WARNING_MESSAGE);
						sound.play();
						t1.setText("");
						pwd.setText("");
		...
		} else if (ae.getSource() == b3) { // cancel
			sound.play();
