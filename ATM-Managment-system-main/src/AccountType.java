	...
	public void actionPerformed(ActionEvent ae) {
		...
		} else if (ae.getSource() == bs1)// saving account matching
		{
			...
			try {
				...
				ps = con.prepareStatement("select acctype from accountdetail where atmno='" + atno + "' and accno='"
						+ acno + "' and pinno='" + pno + "' ");
				rs = ps.executeQuery();
				while (rs.next()) {
				...
				if (foundrec == 1) {
					if (actypegive.equals("saving")) {
						...
						JOptionPane.showMessageDialog(null, "Your account type is not match with given account type.",
